...
/**
 * If `listingUrl` is a single-job board page, try to find a direct employer/ATS apply link.
 * @returns {Promise<{ resolved: string, source: 'ats' | 'original' }>}
 */
async function resolveEmployerListingUrl(listingUrl) {
  const original = String(listingUrl || '').trim();
  if (!original || original === '—') {
    return { resolved: '', source: 'original' };
  }
  if (!isBoardSingleJobUrl(original)) {
    if (isLikelyAtsOrCareersUrl(original) && !isBadAggregatorUrl(original)) {
      return { resolved: original, source: 'original' };
    }
    return { resolved: original, source: 'original' };
  }

  const pageText = await fetchUrlAsPlainText(original);
 ...
 */
async function resolveUrlsOnRows(rows, concurrency) {
  ...
  for (let i = 0; i < list.length; i += n) {
    ...
    const done = await Promise.all(
      chunk.map(async function (row) {
        ...
        const { resolved } = await resolveEmployerListingUrl(u);
