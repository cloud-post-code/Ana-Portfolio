/**
 * Append Job Hunter Open Roles into Job CRM (PostgreSQL or data/jobs-crm.json) with deduping.
 */

const { getJobsCrm, saveJobsCrm } = require('./cms-store');
 ...
 * @returns {Promise<{ added: number, skipped: number, newIds: string[] }>}
 */
async function mergeJobHunterOpenRolesIntoCrm(openRoles) {
  const out = { added: 0, skipped: 0, newIds: [] };
  if (!Array.isArray(openRoles) || !openRoles.length) {
    return out;
  }

  let data = await getJobsCrm();
