...
function provenanceHaystack(row, webSnippets) {
  const ex = row && row._listingExcerpt != null ? String(row._listingExcerpt) : '';
  return String(webSnippets || '') + '\n' + ex;
}
...
function verifyOpenRoleRows(rows, ctx) {
  ...
  return rows.map(function (row, idx) {
    ...
    if (effectiveUrl && /^https?:\/\//i.test(effectiveUrl)) {
      if (isGoogleSearchFallbackUrl(effectiveUrl)) {
        issues.push(
          'Google search URLs cannot be used—Listing must be the exact URL from web snippets or fetched posting text.'
        );
        effectiveUrl = '';
      } else {
        const hay = provenanceHaystack(row, webSnippets);
        if (hay.trim().length > 80 && !urlInText(effectiveUrl, hay)) {
          issues.push(
            'Listing URL must appear verbatim in Tavily snippets or the fetched posting page for this row—unverifiable URL removed.'
          );
          effectiveUrl = '';
        }
      }
    ...
    if (effectiveUrl && /^https?:\/\//i.test(effectiveUrl)) {
      const normU = effectiveUrl.split('?')[0].replace(/\/$/, '').toLowerCase();
      if (seenUrls.has(normU)) {
        issues.push('Same source URL as another row in this table—possible duplicate posting.');
      } else {
        seenUrls.set(normU, idx);
      }
      if (isLikelyBadJobListingUrl(effectiveUrl)) {
        issues.push(
          'URL looks like a search page, job board hub, or non-single-job link—not a direct posting. Verify on the employer site.'
        );
      }
      if (isLikelyGenericCareersOrJobsHub(effectiveUrl)) {
        issues.push(
          'URL looks like a generic careers or jobs landing page—use the ATS or single-job apply link for this role instead.'
        );
      }
      if (co && companySeemsUnrelatedToHost(co, effectiveUrl)) {
        issues.push('Host may not match the stated company—verify employer before applying.');
      }
    } else if (co && role && !effectiveUrl) {
      issues.push('No verified source URL from snippets or fetched page—obtain the posting link before applying.');
    }
    ...
    out.url = effectiveUrl;
    if (Object.prototype.hasOwnProperty.call(out, '_listingExcerpt')) {
      delete out._listingExcerpt;
    }
    if (issues.length) {
