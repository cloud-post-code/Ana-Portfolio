...
async function getPortfolio(collection) {
  if (collection !== 'experiences' && collection !== 'projects' && collection !== 'pdfs') {
    throw new Error('Invalid collection');
  }
  const file = collection === 'experiences' ? 'experiences.json' : (collection === 'pdfs' ? 'pdfs.json' : 'projects.json');
...
/** PostgreSQL-only helpers used by the deploy seed to stay non-destructive. */
async function hasKv(key) {
  if (!isDatabaseEnabled()) return false;
  const r = await getPool().query('SELECT 1 FROM app_kv WHERE key = $1 LIMIT 1', [key]);
  return r.rows.length > 0;
}

async function countPortfolio(collection) {
  if (collection !== 'experiences' && collection !== 'projects' && collection !== 'pdfs') {
    throw new Error('Invalid collection');
  }
  if (!isDatabaseEnabled()) return 0;
  const r = await getPool().query(
    'SELECT COUNT(*)::int AS n FROM portfolio_items WHERE collection = $1',
    [collection]
  );
  return r.rows[0].n;
}
