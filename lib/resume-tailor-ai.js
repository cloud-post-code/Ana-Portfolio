/**
 * Job-tailored resume: Claude writes from profile + experiences + projects, then hiring-manager pass.
 */
...
async function claudeMessage(system, user, maxTokens) {
  ...
  const mt =
    maxTokens != null && Number(maxTokens) > 0
      ? Math.min(Number(maxTokens), 16384)
      : 8000;
...
async function generateTailoredResumeDraft(opts) {
  ...
  return claudeMessage(RESUME_WRITER_SYSTEM, user, draftMax || 8000);
}
...
async function refineResumeWithHiringManager(opts) {
  ...
  return claudeMessage(system, user, Number(refineMax) || 8000);
}
...
/**
 * Master resume: same writer + hiring-manager flow, but not tied to one CRM job.
 * Uses profile (including PDF autofill fields) + selected portfolio experiences/projects.
 *
 * @param {{ profile: object, experiences?: object[], projects?: object[] }} opts
 * @returns {Promise<string>}
 */
async function generateStandaloneResumeAsText(opts) {
  const profile = opts.profile || {};
  const ex = filterPortfolioForResume(opts.experiences || [], profile.resumeIncludedExperienceIds);
  const pr = filterPortfolioForResume(opts.projects || [], profile.resumeIncludedProjectIds);
  const hl = String(profile.headline || '').trim();
  const loc = (profile.applicant && String(profile.applicant.cityState || '').trim()) || '—';
  const job = {
    co: '—',
    role: hl || 'Professional opportunities',
    loc: loc,
    yoe: '—',
    notes:
      'Standalone master resume (not one employer posting). Optimize for ATS and general hiring-manager review. Use structured profile, additional experience notes, and portfolio data; do not imply application to a single company.'
  };
  const baselineSuffix =
    'BASELINE DOCUMENT: This file is the canonical baseline resume (not tied to one job posting). ' +
    'Include complete sections and bullets supported by the input. The document may run multiple pages in Word when the material warrants it—prioritize completeness and clarity over fitting a single page. ' +
    'Job-specific tailoring happens separately when downloading from the Job CRM.';
  ...
  const draft = await generateTailoredResumeDraft({
    ...
    experiences: ex,
    projects: pr,
    userSuffix: baselineSuffix,
    draftMaxTokens: Number(process.env.BASELINE_RESUME_DRAFT_MAX_TOKENS) || 8192
  });
  return refineResumeWithHiringManager({
    job: job,
    draftResumeText: draft,
    maxTokens: Number(process.env.BASELINE_RESUME_REFINE_MAX_TOKENS) || 8192
  });
}

module.exports = {
  generateTailoredResumeForJob,
  generateStandaloneResumeAsText,
