...
function formatStructuredProfileForPrompt(profile) {
  ...
  if (p.summary) lines.push('- Career summary: ' + trimBlock(p.summary, 600));
...
async function claudeMessage(system, user) {
...
async function generateTailoredResumeDraft(opts) {
  ...
  return claudeMessage(RESUME_WRITER_SYSTEM, user);
}

/**
 * @param {{ job: object, draftResumeText: string }} opts
 * @returns {Promise<string>}
 */
async function refineResumeWithHiringManager(opts) {
  ...
  return claudeMessage(system, user);
}
