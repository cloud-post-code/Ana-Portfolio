...
/**
 * Profile notes "Enhance" always uses OpenAI GPT (allowlisted models only).
 * Ignores provider from the client when the selected model is not an OpenAI id (e.g. Claude from the dropdown).
 *
 * @param {{ provider?: string, model?: string } | null | undefined} body
 * @returns {{ provider: 'openai', model: string }}
 */
function resolveResumeNotesEnhanceRequest(body) {
  if (!process.env.OPENAI_API_KEY) {
    throw new Error('OPENAI_API_KEY is required — Enhance uses GPT.');
  }
  const b = body || {};
  let model = String(b.model || '').trim();
  if (!model || !OPENAI_IDS.has(model)) {
    const envM = String(process.env.OPENAI_MODEL || '').trim();
    model = envM && OPENAI_IDS.has(envM) ? envM : DEFAULT_OPENAI_CHAT_MODEL;
  }
  if (!OPENAI_IDS.has(model)) {
    const first = OPENAI_RESUME_MODELS[0];
    model = first ? first.id : 'gpt-4o-mini';
  }
...
module.exports = {
  ...
  resolveResumeNotesEnhanceRequest
};
