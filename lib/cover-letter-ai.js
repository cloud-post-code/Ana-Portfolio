/**
 * AI-written cover letter (Claude) grounded in CRM job + portfolio experiences + profile.
...
function formatProfileForPrompt(profile) {
  ...
  if (p.summary) lines.push('- Career summary / goals: ' + trimBlock(p.summary, 600));
...
async function generateCoverLetterWithAI(opts) {
  ...
  if (!res.ok) {
    ...
    throw new Error('Claude API error ' + res.status + ': ' + errText.slice(0, 240));
  }
...
/**
 * Uses Claude when ANTHROPIC_API_KEY is set and COVER_LETTER_AI is not "0".
...
async function generateCoverLetterSmart(opts) {
  const useAi = process.env.ANTHROPIC_API_KEY && String(process.env.COVER_LETTER_AI || '').trim() !== '0';
  if (!useAi) {
