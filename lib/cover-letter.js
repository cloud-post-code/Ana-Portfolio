...
function scoreExperienceForJob(exp, job) {
  const jobBlob = norm([...(job.tags || []), job.role, job.notes, job.co, job.yoe].join(' '));
  let score = 0;
...
function generateCoverLetter(opts) {
  ...
  const skillHints = [].concat(profile.targetSkillsToUse || []).filter(Boolean);
...
module.exports = { generateCoverLetter, slugify, pickExperiences };
