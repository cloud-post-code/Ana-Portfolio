...
async function gatherWebContext(collection, item) {
  ...
  if (collection === 'jobs-crm' && item.url) {
    if (!isLikelyBadJobListingUrl(item.url)) {
      ...
      if (page) {
        parts.push('=== Listing page (read via stored URL) ===\n' + page);
        if (isBoardSingleJobUrl(item.url)) {
          const ats = extractBestAtsUrlFromPageText(page);
          const cur = String(item.url || '').trim();
          if (ats && ats !== cur) {
            parts.push(
              '=== Resolved direct apply URL (employer/ATS from listing page) ===\nPrefer this for the `url` field if it matches this vacancy: ' +
                ats
            );
          }
        }
  ...
  const jobsCrmExtra =
    collection === 'jobs-crm'
      ? [
          ...
          '- `deadline`: Extract from listing (YYYY-MM-DD) or `Rolling` only if explicitly stated; otherwise "".',
          '- `yoe`: Copy the exact experience requirement phrase from the listing (e.g. "2+ years", "Entry level").',
          '- `notes`: Must be detailed enough for **resume tailoring and cover letter** generation: responsibilities, must-have vs nice-to-have skills, tools/stack, team, work arrangement (remote/hybrid), compensation only if stated, and how to apply. If Context is thin, say so explicitly and avoid invented details.',
