...
const JOB_HUNTER_CRM_TABLE_SPEC =
  ...
  '- **Listing**: the **Apply** link for **this** role—the URL where the candidate starts or submits an application (employer ATS preferred: Greenhouse, Lever, Workday, Ashby, SmartRecruiters, etc.). Use a **single-requisition** URL, not a company-wide careers home, not “all openings” search, not a job-board **search** URL. If snippets show both a board page and an employer apply link for the same vacancy, **prefer the employer/ATS apply URL**. Full `https://…` only when verified in snippets; **Default when unknown:** `—`. Never invent URLs.\n' +
  ...
  '2. **Apply / Listing URL**: The link must take someone to **this job’s application flow** (ATS apply page, or one canonical posting page for that requisition such as LinkedIn `/jobs/view/…` or Indeed `viewjob`). Reject generic `/careers` or `/jobs` landing pages with no job id, job-board **search** URLs, and aggregators’ “similar jobs” pages.\n' +
  '3. **URL provenance**: Prefer URLs that appear in the snippets. Do not paste a URL you did not see in context unless you state clearly it still needs verification.\n' +
