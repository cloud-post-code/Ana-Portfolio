/**
 * Load skills/job-hunter (SKILL.md + references) and call OpenAI or Anthropic with optional Tavily web context.
 */
...
async function runJobHunterSkill(opts) {
  if (!process.env.OPENAI_API_KEY && !process.env.ANTHROPIC_API_KEY) {
    throw new Error(
      'Set OPENAI_API_KEY or ANTHROPIC_API_KEY on the server. Optional: TAVILY_API_KEY for live web snippets.'
    );
  }
  ...
  const out = await completeChatLlm({
    system,
    user: userContent,
    maxTokens,
    temperature: 0.2,
    openaiModel: process.env.OPENAI_MODEL || DEFAULT_OPENAI_CHAT_MODEL,
    anthropicModel: process.env.JOB_HUNTER_MODEL || process.env.ANTHROPIC_MODEL || 'claude-haiku-4-5'
  });
