...
const { fetchUrlAsPlainTextCached } = require('./resolve-employer-url');
...
function mergeEnrichedRow(base, enriched) {
  ...
  if (Object.prototype.hasOwnProperty.call(enriched, 'url')) {
    const vu = enriched.url;
    if (vu == null) {
      out.url = '';
  ...
  const strKeys = ['co', 'role', 'loc', 'notes', 'deadline', 'yoe'];
 ...
 * @param {{ maxRows?: number, listingFetchCache?: Map<string, string>, fetchConcurrency?: number }} [opts] - cap rows; optional shared Jina cache from resolve step; parallel fetch limit
...
async function enrichOpenRolesRows(rows, opts) {
  ...
  const cache = opts && opts.listingFetchCache ? opts.listingFetchCache : undefined;
  const conc =
    opts && typeof opts.fetchConcurrency === 'number' && opts.fetchConcurrency > 0
      ? opts.fetchConcurrency
      : 4;

  const listingBlocks = new Array(slice.length);
  for (let start = 0; start < slice.length; start += conc) {
    const end = Math.min(start + conc, slice.length);
    await Promise.all(
      Array.from({ length: end - start }, function (_, k) {
        const i = start + k;
        return (async function () {
          ...
          if (u && u !== '—' && /^https?:\/\//i.test(u)) {
            page = await fetchUrlAsPlainTextCached(u, cache);
          }
          listingBlocks[i] = {
            originalIdx: i,
            row: row,
  ...
  const system = [
    ...
    'Expand `notes` so it is sufficient for resume tailoring and cover letter writing: aim for **4–8** clause-style segments (semicolon-separated is fine) covering responsibilities, must-have vs nice-to-have qualifications, tools/stack, team if stated, work arrangement (remote/hybrid), comp only if explicitly stated, and how to apply.',
    ...
    'Set `url` to a single job or apply **only** if that exact `https://…` string appears **verbatim** in the `listingPageExcerpt` (preferred) or is already in the row snapshot and still valid. Prefer ATS or apply URLs from the excerpt over generic hubs when the excerpt contains them. **Never** output `google.com/search` or any URL not present verbatim in the excerpt or snapshot. If you cannot ground a URL in the excerpt, return an **empty string** for `url`.',
  ...
  for (let i = 0; i < slice.length; i++) {
    ...
    const lb = listingBlocks[i];
    if (lb) {
      out[i]._listingExcerpt = lb.pageExcerpt || '';
    }
  }
  return out;
}
