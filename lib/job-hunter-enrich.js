...
async function enrichOpenRolesRows(rows, opts) {
  ...
  const system = [
    ...
    'Set `url` to the best **apply** destination for this vacancy: prefer employer ATS or the explicit apply URL from the excerpt over generic career hubs or board search pages. If the excerpt shows a Greenhouse/Lever/Workday (etc.) link for this requisition, use that. If only a single-job board URL (e.g. LinkedIn job view) is supported by the excerpt, keep it; do not swap in a company-wide /careers home.',
