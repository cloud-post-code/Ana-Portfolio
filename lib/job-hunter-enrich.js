...
async function enrichOpenRolesRows(rows, opts) {
  ...
  const system = [
    ...
    'Set `url` to the best **apply** or single-job listing URL: prefer employer ATS or explicit apply URL from the excerpt over hubs. If the input `url` is a `google.com/search` link (last-resort fallback), replace it **only** if the excerpt or snippets clearly provide a better direct URL; otherwise keep the Google search link.',
