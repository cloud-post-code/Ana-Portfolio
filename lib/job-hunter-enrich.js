...
async function completeAnthropicEnrich(system, user) {
...
async function completeOpenAIEnrich(system, user) {
 ...
 */
async function enrichOpenRolesRows(rows) {
  if (
  ...
  const slice = rows.slice(0, MAX_ROWS_ENRICH);
  ...
  try {
    if (process.env.ANTHROPIC_API_KEY) {
      parsed = await completeAnthropicEnrich(system, user);
    } else if (process.env.OPENAI_API_KEY) {
      parsed = await completeOpenAIEnrich(system, user);
    } else {
      return rows;
    }
  } catch (e) {
...
module.exports = { enrichOpenRolesRows, MAX_ROWS_ENRICH };
